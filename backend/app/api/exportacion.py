...
def facturas_pendientes_exportacion(empresa_id: str, plantilla_id: str, db: Session = Depends(get_db),
    ...
    cfgs = configuraciones_empresa(db, empresa) if str(getattr(sistema, "value", sistema)) == "siigo_pyme" else {}
    return [{
        ...
        "tipo_documento_clave": tipo_documento_clave(f),
        "modo_numeracion": (cfgs.get(tipo_documento_clave(f), {}) or {}).get("modo_numeracion", "interna"),
        "tipo_comprobante": (cfgs.get(tipo_documento_clave(f), {}) or {}).get("tipo_comprobante"),
        "codigo_comprobante": (cfgs.get(tipo_documento_clave(f), {}) or {}).get("codigo_comprobante"),
...
def eliminar_plantilla(empresa_id: str, plantilla_id: str, db: Session = Depends(get_db),
    ...
    ya_usada = db.query(Exportacion).filter(Exportacion.empresa_id == empresa_id, Exportacion.plantilla_id == plantilla_id).first()
    if ya_usada:
...
def previsualizar_exportacion(empresa_id: str, payload: GenerarExportacionRequest, db: Session = Depends(get_db),
    ...
    numeros = proyectar_numeros(db, empresa, facturas_ordenadas, payload.consecutivo_inicial, payload.consecutivos_iniciales) if sistema == "siigo_pyme" else None
    ...
    for f in facturas:
        for m in db.query(Movimiento).filter(Movimiento.empresa_id == empresa_id, Movimiento.factura_id == f.id).all():
...
def generar_exportacion(empresa_id: str, payload: GenerarExportacionRequest, db: Session = Depends(get_db),
    ...
    try:
        ...
        cfg_por_factura = {}
        if sistema == "siigo_pyme":
            numeros, cfg_por_factura = asignar_numeros(db, empresa, facturas_ordenadas, payload.consecutivo_inicial, payload.consecutivos_iniciales)
        ...
        auditoria_registrar(db, empresa_id, "Exportacion", exportacion.id, "exportacion_generada",
                             {"archivo": nombre_archivo, "registros": total_filas,
                              ...
                              "consecutivo_inicial": payload.consecutivo_inicial, "consecutivos_iniciales": payload.consecutivos_iniciales},
