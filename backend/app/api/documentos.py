...
from app.schemas.schemas import (
    ...
    RelacionDocumentoRequest, AplicarRetencionesRequest,
    GenerarPartidaRequest, PartidaOut, LineaPartidaOut,
)
from app.services.zip_processing_service import procesar_archivos_mixtos
from app.services import documentos_service, partida_doble_service, historial_service, retenciones_service
...
# La instancia Free de Render tiene recursos muy limitados. La carga DIAN es
# una operación CPU/RAM intensiva (XML/PDF/Excel + conciliación + BD). Permitir
# dos cargas pesadas simultáneas en el mismo proceso puede disparar memoria y
# provocar un reinicio del contenedor. Se serializan las cargas pesadas por
# proceso; el resto de la API (incluido /salud) sigue atendiendo normalmente.
_CARGA_PESADA_LOCK = threading.Lock()
...
def listar_cargas(empresa_id: str, db: Session = Depends(get_db),
    ...
    return [
        {
            "id": c.id, "archivo_zip_nombre": c.archivo_zip_nombre,
            "archivo_excel_nombre": c.archivo_excel_nombre, "creado_en": c.creado_en,
            "total_relacionados": c.total_relacionados,
            "total_pendientes_revision": c.total_pendientes_revision,
            "total_pendientes_clasificacion": c.total_pendientes_clasificacion,
            "total_duplicados": c.total_duplicados,
            "total_documentos": db.query(Factura).filter(Factura.empresa_id == empresa_id, Factura.carga_id == c.id).count(),
        }
        for c in cargas
    ]
...
def eliminar_carga_completa(empresa_id: str, carga_id: str, db: Session = Depends(get_db),
                             empresa: Empresa = Depends(get_empresa_activa),
                             usuario: str = Depends(usuario_actual),
                             actor: Usuario | None = Depends(get_current_user)):
    """Revierte una sesión de carga DIAN completa sin tocar cargas anteriores.

    La eliminación se hace explícitamente en orden hijo -> padre para respetar
    las llaves foráneas de PostgreSQL/Neon. Todos los filtros incluyen
    ``empresa_id``: una carga nunca puede retirar documentos de otra empresa.

    Si algún documento ya fue exportado se exige el Administrador General,
    igual que en la eliminación individual. La exportación histórica y la
    auditoría se conservan; solo se retiran las relaciones operativas.
    """
    carga = db.query(CargaDocumentosDian).filter(
        CargaDocumentosDian.empresa_id == empresa_id,
        CargaDocumentosDian.id == carga_id,
    ).first()
    if not carga:
        raise HTTPException(status_code=404, detail="Carga no encontrada en esta empresa.")

    # Si una regresión antigua dejó una factura de otra empresa apuntando a
    # esta carga, no intentamos corregirla silenciosamente desde aquí. Bloquear
    # es más seguro que borrar información cruzada entre clientes.
    referencias_cruzadas = db.query(Factura.id).filter(
        Factura.carga_id == carga_id,
    ...
    if referencias_cruzadas:
        raise HTTPException(
            status_code=409,
            detail=(
                "Se detectó una inconsistencia multiempresa en esta carga. "
                "No se eliminó nada para proteger los datos de los otros clientes."
            ),
        )

    # Consultamos únicamente los IDs para no dejar objetos Factura persistentes
    # en la sesión mientras hacemos los DELETE masivos.
    ids = [row[0] for row in db.query(Factura.id).filter(
        Factura.empresa_id == empresa_id,
        Factura.carga_id == carga_id,
    ).all()]
    ...
    if ids:
        relaciones = db.query(ExportacionFactura).filter(
            ...
            ExportacionFactura.factura_id.in_(ids),
        ).all()

    if relaciones and actor is not None and not actor.es_superadmin:
        raise HTTPException(
            status_code=403,
            detail="La carga contiene documentos ya exportados. Solo el Administrador General puede retirarla conservando la trazabilidad.",
        )

    exportadas = {r.factura_id for r in relaciones}
    ...
    resumen = {
        "archivo_excel": carga.archivo_excel_nombre,
        "archivo_zip": carga.archivo_zip_nombre,
        "documentos_eliminados": len(ids),
        "exportados_retirados": len(exportadas),
        "exportaciones_previas": exportaciones_previas,
    }

    try:
        if ids:
            # 1) Relaciones de exportación -> Factura. La cabecera de la
            # exportación se conserva como trazabilidad histórica.
            db.query(ExportacionFactura).filter(
                ExportacionFactura.empresa_id == empresa_id,
                ExportacionFactura.factura_id.in_(ids),
            ).delete(synchronize_session=False)

            # 2) Liberar documentos de ESTA empresa que apunten como duplicado
            # a alguno de los documentos que vamos a retirar.
            db.query(Factura).filter(
                Factura.empresa_id == empresa_id,
                Factura.duplicado_de_id.in_(ids),
                ~Factura.id.in_(ids),
            ).update({
            ...
            # 3) Movimientos contables dependientes -> Factura.
            db.query(Movimiento).filter(
                Movimiento.empresa_id == empresa_id,
                Movimiento.factura_id.in_(ids),
            ).delete(synchronize_session=False)

            # 4) Facturas de la carga. Este DELETE se ejecuta antes de borrar
            # la carga padre, evitando la ForeignKeyViolation observada en Neon.
            db.query(Factura).filter(
                Factura.empresa_id == empresa_id,
                Factura.id.in_(ids),
                Factura.carga_id == carga_id,
            ...
            # Fuerza el orden antes del DELETE de cargas_documentos_dian.
            db.flush()

            # Invariante de seguridad: no debe quedar ninguna factura de esta
            # empresa referenciando la carga que vamos a retirar.
            restantes = db.query(Factura.id).filter(
                Factura.empresa_id == empresa_id,
                Factura.carga_id == carga_id,
            ).count()
            if restantes:
                raise HTTPException(
                    status_code=409,
                    detail=f"No se pudo eliminar la carga de forma segura: aún quedan {restantes} documento(s) asociados.",
                )

        # 5) Solo después de retirar todos los hijos se elimina la carga padre.
        db.delete(carga)
        db.flush()

        auditoria_registrar(
            db, empresa_id, "CargaDocumentosDian", carga_id,
            "carga_documentos_eliminada", resumen, usuario,
        )
        db.commit()
        ...
        raise HTTPException(
            status_code=409,
            detail=(
                "No se pudo eliminar la carga porque todavía existe información relacionada. "
                "No se borró nada. Actualiza la página y vuelve a intentarlo; si persiste, "
                "revisa los documentos asociados a esta carga."
            ),
        )
...
def cargar_documentos(
    ...
    # Las dependencias de autenticación/empresa ya consultaron PostgreSQL antes
    # de entrar aquí. No debemos conservar esa conexión prestada mientras se
    # analizan XML/PDF/Excel (ni mientras esperamos el lock de otra carga): Neon
    # puede cerrar el socket por inactividad y el primer INSERT terminaría con
    # ``SSL connection has been closed unexpectedly``. Guardamos únicamente el
    # dato escalar que necesita el parser y devolvemos la conexión al pool. La
    # misma Session es reutilizable: cuando comience la fase transaccional pedirá
    # una conexión fresca y ``pool_pre_ping`` la validará antes de usarla.
    nit_empresa = empresa.nit
    db.close()

    # IMPORTANTE: este endpoint es deliberadamente síncrono. FastAPI ejecuta
    # las rutas ``def`` en su threadpool, de modo que pdfplumber/pandas/XML y
    # SQLAlchemy no bloquean el event loop que debe contestar el health check
    # de Render. La versión anterior era ``async def`` pero hacía casi todo el
    # trabajo CPU/síncrono dentro del event loop; durante una carga el /salud
    # dejaba de responder y Render podía reiniciar el servicio (HTTP 503).
    inicio = time.monotonic()
    nombres_entrada = [f.filename or "archivo_sin_nombre" for f in (documentos or [])]
    logger.info(
        "Carga DIAN iniciada empresa=%s documentos=%s excel=%s",
        empresa_id, len(nombres_entrada), excel_file.filename if excel_file else None,
    )

    # En una instancia pequeña preferimos esperar a que termine otra carga
    # pesada antes que competir por RAM/CPU y tumbar el proceso completo.
    with _CARGA_PESADA_LOCK:
        archivos_leidos = []
        total_bytes_entrada = 0
        for f in (documentos or []):
            # UploadFile usa un SpooledTemporaryFile. Al estar ya dentro del
            # threadpool, se lee de forma síncrona sin bloquear el event loop.
            f.file.seek(0)
            contenido = f.file.read()
            total_bytes_entrada += len(contenido)
            archivos_leidos.append((f.filename or "archivo_sin_nombre", contenido))

        try:
            documentos_zip = procesar_archivos_mixtos(archivos_leidos, nit_empresa=nit_empresa)
        except Exception as e:
            logger.exception("Fallo leyendo documentos DIAN empresa=%s", empresa_id)
            raise HTTPException(status_code=422, detail=f"No se pudieron leer los archivos: {e}")
        finally:
            # Los ZIP/XML/PDF originales ya fueron interpretados. Liberarlos
            # antes de abrir el Excel reduce el pico de memoria de la carga.
            archivos_leidos.clear()
            gc.collect()
        ...
        if excel_file is not None:
            ...
            excel_contenido = excel_file.file.read()
            total_bytes_entrada += len(excel_contenido)
            mapeo = {
        ...
        # A partir de aquí comienza la fase de base de datos. Como la Session
        # se cerró antes del procesamiento pesado, este primer flush adquiere
        # una conexión NUEVA del pool en vez de reutilizar el socket que estuvo
        # ocioso durante la lectura de documentos.
        carga = CargaDocumentosDian(
            empresa_id=empresa_id,
            archivo_excel_nombre=excel_file.filename if excel_file else None,
            archivo_zip_nombre=(", ".join(nombres_entrada)[:300] or None),
            usuario=usuario,
        )
        db.add(carga)
        db.flush()
        ...
        try:
            ...
            db.rollback()
            logger.warning("Carga DIAN rechazada empresa=%s motivo=%s", empresa_id, e)
            raise HTTPException(status_code=422, detail=str(e))
        except Exception:
            db.rollback()
            logger.exception("Carga DIAN falló durante conciliación/BD empresa=%s", empresa_id)
            raise HTTPException(
                status_code=500,
                detail=(
                    "La carga no pudo completarse y fue revertida de forma segura. "
                    "No se guardó información parcial; revisa el log del servidor para el detalle técnico."
                ),
            )

        # Invariante multiempresa: una sesión de carga solo puede producir
        # documentos pertenecientes a la misma empresa del URL. Esta comprobación
        # es redundante con la creación normal, pero evita que una regresión futura
        # pueda mezclar datos entre clientes sin detectarse.
        cruzadas = db.query(Factura).filter(
            Factura.carga_id == carga.id,
            Factura.empresa_id != empresa_id,
        ).count()
        if cruzadas:
            db.rollback()
            raise HTTPException(
                status_code=500,
                detail="Se detectó una inconsistencia de aislamiento multiempresa. La carga fue cancelada.",
            )
        ...
        logger.info(
            "Carga DIAN completada empresa=%s carga=%s documentos=%s bytes=%s duracion_s=%s",
            empresa_id, carga.id, len(documentos_zip), total_bytes_entrada, duracion,
        )
        # El DataFrame ya fue consumido dentro de procesar_carga. Soltar los
        # bytes del Excel y las estructuras extraídas antes de serializar la
        # respuesta ayuda especialmente en la instancia Free.
        excel_contenido = None
        gc.collect()
...
def panel_clasificacion(empresa_id: str, modulo: str | None = None,
                         ...
                         naturaleza: str | None = None, estado: str | None = None,
                         db: Session = Depends(get_db),
                         empresa: Empresa = Depends(get_empresa_activa)):
    """
    ...
    for f in facturas:
        base = {"id": f.id, "tercero_nombre": f.tercero_nombre, "tercero_nit": f.tercero_nit,
                ...
                "naturaleza_documento": f.naturaleza_documento,
                "direccion_documento": f.direccion_documento}

        motivos_revision = []
        if f.alertas:
            motivos_revision.append("; ".join(f.alertas[:3]))
        if f.naturaleza_documento in ("nota_credito", "nota_debito") and not getattr(f, "documento_origen_id", None):
            motivos_revision.append(getattr(f, "relacion_documento_motivo", None) or "Nota sin factura original relacionada.")
        if f.naturaleza_documento == "nomina":
            motivos_revision.append("Nómina electrónica — requiere registro manual.")
        if not f.tercero_nit:
            motivos_revision.append("Sin NIT de tercero identificado.")

        # Una partida ya generada solo entra a “Listas para aprobar” si no
        # conserva una excepción relevante. Por ejemplo, una nota crédito sin
        # factura original vinculada debe seguir visible en Requieren revisión.
        if f.estado == EstadoFactura.lista_para_contabilizar and not motivos_revision:
            listas.append(base)
            continue

        sug = sugerencias_masivas.get(f.id)
        nivel = (sug or {}).get("nivel_confianza") or "baja"
        if sug and sug.get("cuenta_sugerida") and nivel == "alta" and not motivos_revision:
            motivo = (f"Historial: {sug['usos']} de {sug['total']} uso(s) para este contexto "
                      f"({sug['porcentaje']}%).")
            con_sugerencia.append({**base, "cuenta_sugerida": sug["cuenta_sugerida"],
                                    "motivo_sugerencia": motivo, "fuente_sugerencia": sug["fuente"],
                                    "nivel_confianza": nivel, "puntaje_confianza": sug.get("puntaje_confianza", 0)})
            continue

        if sug and nivel == "media":
            motivos_revision.append(
                f"Sugerencia con confianza media: {sug.get('cuenta_sugerida') or sug.get('cuenta_candidata') or 'cuenta por revisar'} "
                f"({sug.get('porcentaje', 0)}% del historial considerado)."
            )
        elif sug and nivel == "baja":
            motivos_revision.append("Historial ambiguo o insuficiente; no se selecciona una cuenta automáticamente.")
        elif not sug and not motivos_revision:
            motivos_revision.append("Sin sugerencia suficientemente respaldada por el historial real.")

        necesita_revision.append({**base, "motivo": " ".join(motivos_revision),
                                   "nivel_confianza": nivel,
                                   "cuenta_candidata": (sug or {}).get("cuenta_candidata")})
...
def obtener_factura(empresa_id: str, factura_id: str, db: Session = Depends(get_db),
    ...
    return {
        ...
        "retenciones_contables": json.loads(f.retenciones_contables_json) if getattr(f, "retenciones_contables_json", None) else {},
        "retenciones_efectivas": retenciones_service.retenciones_efectivas(f),
        "documento_origen_id": getattr(f, "documento_origen_id", None),
        "relacion_documento_confianza": getattr(f, "relacion_documento_confianza", None),
        "relacion_documento_motivo": getattr(f, "relacion_documento_motivo", None),
...
@router.get("/{factura_id}/documentos-origen-candidatos")
def candidatos_documento_origen(empresa_id: str, factura_id: str, db: Session = Depends(get_db),
                                empresa: Empresa = Depends(get_empresa_activa)):
    nota = db.query(Factura).filter(Factura.empresa_id == empresa_id, Factura.id == factura_id).first()
    if not nota:
        raise HTTPException(status_code=404, detail="Documento no encontrado.")
    if nota.naturaleza_documento not in ("nota_credito", "nota_debito"):
        return []
    q = db.query(Factura).filter(
        Factura.empresa_id == empresa_id,
        Factura.id != nota.id,
        Factura.direccion_documento == nota.direccion_documento,
        Factura.naturaleza_documento.in_(["factura", "documento_equivalente"]),
    )
    candidatos = q.all()
    if nota.tercero_nit:
        mismos = [x for x in candidatos if x.tercero_nit == nota.tercero_nit]
        if mismos:
            candidatos = mismos
    candidatos = sorted(candidatos, key=lambda x: x.fecha_emision or x.creado_en, reverse=True)[:30]
    return [{"id": x.id, "prefijo": x.prefijo, "numero_factura": x.numero_factura,
             "fecha_emision": x.fecha_emision, "tercero_nombre": x.tercero_nombre,
             "total": float(x.total or 0)} for x in candidatos]


@router.patch("/{factura_id}/relacion-documento")
def relacionar_documento_manual(empresa_id: str, factura_id: str, payload: RelacionDocumentoRequest,
                                db: Session = Depends(get_db), empresa: Empresa = Depends(get_empresa_activa),
                                usuario: str = Depends(usuario_actual)):
    nota = db.query(Factura).filter(Factura.empresa_id == empresa_id, Factura.id == factura_id).first()
    if not nota:
        raise HTTPException(status_code=404, detail="Documento no encontrado.")
    if nota.naturaleza_documento not in ("nota_credito", "nota_debito"):
        raise HTTPException(status_code=422, detail="Solo las notas crédito/débito se relacionan con una factura original.")
    origen = None
    if payload.documento_origen_id:
        origen = db.query(Factura).filter(Factura.empresa_id == empresa_id, Factura.id == payload.documento_origen_id).first()
        if not origen or origen.naturaleza_documento not in ("factura", "documento_equivalente"):
            raise HTTPException(status_code=422, detail="La factura original seleccionada no es válida.")
        nota.documento_origen_id = origen.id
        nota.relacion_documento_confianza = "manual"
        nota.relacion_documento_motivo = f"Relación confirmada manualmente con {origen.prefijo or ''}-{origen.numero_factura or ''}."
    else:
        nota.documento_origen_id = None
        nota.relacion_documento_confianza = None
        nota.relacion_documento_motivo = "Relación retirada manualmente; requiere revisión."
    auditoria_registrar(db, empresa_id, "Factura", nota.id, "relacion_documento_original",
                        {"documento_origen_id": nota.documento_origen_id}, usuario)
    db.commit()
    return {"ok": True, "documento_origen_id": nota.documento_origen_id,
            "motivo": nota.relacion_documento_motivo}


@router.get("/{factura_id}/retenciones-sugeridas")
def sugerir_retenciones_factura(empresa_id: str, factura_id: str, cuenta_codigo: str | None = None,
                                db: Session = Depends(get_db), empresa: Empresa = Depends(get_empresa_activa)):
    f = db.query(Factura).filter(Factura.empresa_id == empresa_id, Factura.id == factura_id).first()
    if not f:
        raise HTTPException(status_code=404, detail="Factura no encontrada.")
    return retenciones_service.sugerir_retenciones(db, empresa, f, cuenta_codigo)


@router.post("/{factura_id}/retenciones-aplicar")
def aplicar_retenciones_factura(empresa_id: str, factura_id: str, payload: AplicarRetencionesRequest,
                                db: Session = Depends(get_db), empresa: Empresa = Depends(get_empresa_activa),
                                usuario: str = Depends(usuario_actual)):
    f = db.query(Factura).filter(Factura.empresa_id == empresa_id, Factura.id == factura_id).first()
    if not f:
        raise HTTPException(status_code=404, detail="Factura no encontrada.")
    # Una retención confirmada cambia la distribución de la partida. Si el documento
    # ya fue exportado, no se permite alterar esa decisión desde aquí; debe retirarse
    # mediante el flujo administrativo correspondiente. Si solo tenía una partida
    # propuesta/no exportada, esa partida se invalida para obligar a regenerarla con
    # las retenciones efectivas correctas.
    ya_exportada = db.query(ExportacionFactura.id).filter(
        ExportacionFactura.empresa_id == empresa_id, ExportacionFactura.factura_id == f.id
    ).first()
    if ya_exportada:
        raise HTTPException(status_code=422, detail="El documento ya fue exportado. Retíralo del flujo de exportación antes de cambiar retenciones contables.")

    movimientos_existentes = db.query(Movimiento).filter(Movimiento.empresa_id == empresa_id, Movimiento.factura_id == f.id).count() > 0

    if not payload.tipos:
        f.retenciones_contables_json = "{}"
        if movimientos_existentes:
            db.query(Movimiento).filter(Movimiento.empresa_id == empresa_id, Movimiento.factura_id == f.id).delete(synchronize_session=False)
            f.estado = EstadoFactura.clasificada
        auditoria_registrar(db, empresa_id, "Factura", f.id, "retenciones_contables_retiradas",
                            {"origen": "revision_usuario", "partida_invalidada": movimientos_existentes}, usuario)
        db.commit()
        return {"ok": True, "retenciones_contables": {}, "aplicadas": {},
                "partida_invalidada": movimientos_existentes}

    sugeridas = retenciones_service.sugerir_retenciones(db, empresa, f, payload.cuenta_principal_codigo)
    por_tipo = {x["tipo"]: x for x in sugeridas.get("sugerencias", [])}
    aplicadas = {}
    for tipo in payload.tipos:
        if tipo not in ("retefuente", "reteica", "reteiva") or tipo not in por_tipo:
            continue
        sug = por_tipo[tipo]
        if sug["nivel_confianza"] == "baja":
            raise HTTPException(status_code=422, detail=f"{tipo}: la confianza es baja; requiere revisión manual.")
        if sug["nivel_confianza"] == "media" and not payload.confirmar_media:
            raise HTTPException(status_code=422, detail=f"{tipo}: confianza media. Confirma explícitamente antes de aplicarla.")
        aplicadas[tipo] = float(sug["valor"])
    try:
        actuales = json.loads(f.retenciones_contables_json or "{}")
    except Exception:
        actuales = {}
    actuales.update(aplicadas)
    f.retenciones_contables_json = json.dumps(actuales, ensure_ascii=False)
    if movimientos_existentes:
        db.query(Movimiento).filter(Movimiento.empresa_id == empresa_id, Movimiento.factura_id == f.id).delete(synchronize_session=False)
        f.estado = EstadoFactura.clasificada
    auditoria_registrar(db, empresa_id, "Factura", f.id, "retenciones_contables_confirmadas",
                        {"retenciones": aplicadas, "origen": "historial_empresa_tercero",
                         "partida_invalidada": movimientos_existentes}, usuario)
    db.commit()
    return {"ok": True, "retenciones_contables": actuales, "aplicadas": aplicadas,
            "partida_invalidada": movimientos_existentes}
...
def _aplicar_partida_a_factura(db: Session, empresa: Empresa, empresa_id: str, f: Factura,
    ...
    # Las correcciones manuales de IVA/retenciones pueden escribirse por
    # código aunque esa cuenta todavía no exista en el catálogo aprendido.
    # Igual que con la cuenta principal, se crea la cuenta técnica por código
    # para respetar la decisión explícita del contador.
    cuentas_control = dict(cuentas_control or {})
    for _rol, _codigo in list(cuentas_control.items()):
        _codigo = str(_codigo or "").strip()
        if not _codigo:
            cuentas_control.pop(_rol, None)
            continue
        historial_service.get_or_create_cuenta(db, empresa_id, _codigo)
    ...
    for fid in factura_ids:
        ...
        if usar_sugerencia:
            ...
            if not f.tercero_nit:
                ...
                continue
            sug = sugerencias_masivas.get(f.id)
            if not sug or not sug.get("cuenta_sugerida") or sug.get("nivel_confianza") != "alta":
                nivel = (sug or {}).get("nivel_confianza", "baja")
                resultados.append({"factura_id": fid, "estado": "omitida",
                                    "motivo": f"Sugerencia con confianza {nivel}; la aplicación masiva solo usa Alta confianza."})
                continue
...
def obtener_partida(empresa_id: str, factura_id: str, db: Session = Depends(get_db),
    ...
    if not f:
        raise HTTPException(status_code=404, detail="Factura no encontrada en esta empresa.")
    movimientos = (
        db.query(Movimiento).filter(Movimiento.empresa_id == empresa_id, Movimiento.factura_id == factura_id).order_by(Movimiento.orden).all()
    )
    if not movimientos:
...
def eliminar_factura(empresa_id: str, factura_id: str, motivo: str | None = None,
    ...
    db.query(Movimiento).filter(Movimiento.empresa_id == empresa_id, Movimiento.factura_id == factura_id).delete(synchronize_session=False)
    db.delete(f)
    auditoria_registrar(
