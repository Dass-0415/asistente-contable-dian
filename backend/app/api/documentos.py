...
def eliminar_carga_completa(empresa_id: str, carga_id: str, db: Session = Depends(get_db),
    ...
    if ids:
        ...
        db.query(Movimiento).filter(Movimiento.factura_id.in_(ids)).delete(synchronize_session=False)
        for f in facturas:
            db.delete(f)
...
def aplicar_retenciones_factura(empresa_id: str, factura_id: str, payload: AplicarRetencionesRequest,
    ...
    movimientos_existentes = db.query(Movimiento).filter(Movimiento.factura_id == f.id).count() > 0
