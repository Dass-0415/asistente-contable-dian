...
def eliminar_carga_completa(empresa_id: str, carga_id: str, db: Session = Depends(get_db),
    ...
    Se eliminan únicamente las facturas cuyo ``carga_id`` coincide con la
    sesión seleccionada. Si algún documento ya fue exportado se exige el
    Administrador General, igual que en la eliminación individual. Los
    registros históricos de exportación y auditoría se conservan.
    """
    carga = db.query(CargaDocumentosDian).filter(
        CargaDocumentosDian.empresa_id == empresa_id, CargaDocumentosDian.id == carga_id
    ).first()
    if not carga:
        raise HTTPException(status_code=404, detail="Carga no encontrada en esta empresa.")

    facturas = db.query(Factura).filter(
        Factura.empresa_id == empresa_id, Factura.carga_id == carga_id
    ).all()
    ids = [f.id for f in facturas]
    relaciones = []
    if ids:
        relaciones = db.query(ExportacionFactura).filter(
            ExportacionFactura.empresa_id == empresa_id, ExportacionFactura.factura_id.in_(ids)
        ).all()
    ...
    exportaciones_previas = [{
        "factura_id": r.factura_id, "exportacion_id": r.exportacion_id,
        "sistema_contable": r.sistema_contable, "tipo_comprobante": r.tipo_comprobante,
        "codigo_comprobante": r.codigo_comprobante, "numero_documento": r.numero_documento,
    } for r in relaciones]
    for r in relaciones:
        db.delete(r)
    ...
    if ids:
        ...
        db.query(Movimiento).filter(Movimiento.empresa_id == empresa_id, Movimiento.factura_id.in_(ids)).delete(synchronize_session=False)
        for f in facturas:
            db.delete(f)

    resumen = {
        "archivo_excel": carga.archivo_excel_nombre, "archivo_zip": carga.archivo_zip_nombre,
        "documentos_eliminados": len(ids), "exportados_retirados": len(exportadas),
        "exportaciones_previas": exportaciones_previas,
    }
    db.delete(carga)
    auditoria_registrar(db, empresa_id, "CargaDocumentosDian", carga_id, "carga_documentos_eliminada", resumen, usuario)
    db.commit()
    return {
        "eliminada": True, "carga_id": carga_id, "documentos_eliminados": len(ids),
        "exportados_retirados": len(exportadas), "trazabilidad_conservada": bool(relaciones),
    }
