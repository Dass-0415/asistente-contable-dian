    ...
    for nota in nuevas:
        ...
        for f in candidatos:
            ...
            if ref_num and ref_num in _referencias_factura(f):
                score += 45
            if nota.fecha_emision and f.fecha_emision and f.fecha_emision <= nota.fecha_emision:
                dias = (nota.fecha_emision.date() - f.fecha_emision.date()).days
                if 0 <= dias <= 120:
                    score += 10
        ...
        puntajes.sort(key=lambda x: (-x[0], x[1].fecha_emision or datetime.min), reverse=False)
        if puntajes:
