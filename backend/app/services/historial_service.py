...
def _palabras_clave(t: str, largo_minimo: int = 4) -> set:
    """Palabras útiles del concepto; referencias/códigos puros no mandan la clasificación."""
    import re
    palabras = set()
    for p in _normalizar_texto(t).split():
        if len(p) < largo_minimo:
            continue
        # Ítems XML como 00192837, REF-12345 o códigos alfanuméricos con muchos
        # dígitos son señales secundarias demasiado débiles para cambiar una cuenta.
        digitos = sum(c.isdigit() for c in p)
        letras = sum(c.isalpha() for c in p)
        if p.isdigit() or (digitos >= 3 and letras <= 3):
            continue
        palabras.add(p)
    return palabras


def _nivel_confianza_historial(fuente: str, usos: int, total: int, porcentaje: float) -> tuple[str, int]:
    """Convierte la evidencia histórica en Alta/Media/Baja sin inventar certeza."""
    if total <= 0 or usos <= 0:
        return "baja", 0
    if fuente == "historial_nit_concepto":
        if usos >= 3 and porcentaje >= 70:
            return "alta", 95
        if usos >= 2 and porcentaje >= 60:
            return "alta", 88
        if porcentaje >= 50:
            return "media", 70
        return "baja", 40
    # Frecuencia general del proveedor: exige más evidencia para ser alta.
    if usos >= 4 and porcentaje >= 80:
        return "alta", 90
    if usos >= 2 and porcentaje >= 65:
        return "media", 68
    return "baja", 35
...
def sugerir_cuenta(db: Session, empresa_id: str, nit: str,
    ...
    if proveedor:
        ...
        registros_filtrados = []
        for r in registros:
            cuenta_hist = db.get(CuentaContable, r.cuenta_id)
            if cuenta_hist and _es_cuenta_de_resultado(cuenta_hist.codigo, clases_permitidas):
        ...
        if registros:
            ...
            if claves_concepto:
                ...
                # Los ítems/descripciones del XML son una señal SECUNDARIA. Un solo
                # match no desplaza el patrón general del proveedor; exigimos al menos
                # dos antecedentes similares para afinar por concepto.
            ...
            nivel, puntaje = _nivel_confianza_historial(
                fuente, principal["usos"], total_considerado, principal["porcentaje"]
            )
            if nivel == "baja":
                motivo += " La evidencia no es suficiente para seleccionar una cuenta automáticamente; requiere revisión."
            return {
                "proveedor_nit": nit,
                "proveedor_nombre": proveedor.nombre,
                "total_documentos_historicos": total,
                "opciones": opciones,
                "cuenta_sugerida": principal["cuenta_codigo"] if nivel != "baja" else None,
                "cuenta_candidata": principal["cuenta_codigo"],
                "motivo": motivo,
                "fuente": fuente,
    ...
    if regla:
        ...
        return {
            ...
            "fuente": "regla",
            "nivel_confianza": "alta", "puntaje_confianza": 100,
        }
        ...
        if claves:
            ...
            if candidatos_propios:
                return {
                    ...
                    "nivel_confianza": "baja", "puntaje_confianza": 25,
                }
    ...
    return {
        ...
        "nivel_confianza": "baja", "puntaje_confianza": 0,
    }
...
def sugerir_cuentas_masivo(db: Session, empresa_id: str, facturas: list) -> dict[str, dict]:
    ...
    for f in validas:
        ...
        nivel, puntaje = _nivel_confianza_historial(fuente, usos, total, porcentaje)
        resultado[f.id] = {
            "cuenta_sugerida": cta.codigo if nivel != "baja" else None,
            "cuenta_candidata": cta.codigo,
            "cuenta_nombre": cta.nombre,
            "fuente": fuente,
            "usos": usos,
            "total": total,
