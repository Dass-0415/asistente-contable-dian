...
from app.services.retenciones_service import retenciones_efectivas
...
def _buscar_cuenta_control_por_nombre(db: Session, empresa: Empresa, rol: str) -> Optional[CuentaContable]:
    ...
    patrones = {
        "iva_descontable": re.compile(r"iva.*descont|descont.*iva|iva.*compr", re.I),
        "iva_generado": re.compile(r"iva.*generad|generad.*iva|iva.*venta|iva.*por pagar", re.I),
    ...
    if not coinc:
        prefijos = {
            "iva_descontable": ("2408",), "iva_generado": ("2408",),
...
def _seleccionar_cuenta_iva_por_tasa(db: Session, empresa: Empresa, subtotal: float, iva: float,
    ...
    if coincidencias:
        ...
        patron_contrario = _PALABRA_DESCONTABLE if tipo_iva == "generado" else _PALABRA_GENERADO
        con_polaridad = [c for c in coincidencias if patron_polaridad.search(c.nombre or "")]
        if con_polaridad:
            coincidencias = con_polaridad
        else:
            # Una cuenta claramente del IVA contrario nunca se usa solo porque
            # coincida la tasa. Conservamos únicamente cuentas genéricas.
            coincidencias = [c for c in coincidencias if not patron_contrario.search(c.nombre or "")]
...
def _generar_partida_compra(db: Session, empresa: Empresa, factura: Factura,
    ...
    retenciones = retenciones_efectivas(factura)
    ...
    if iva > 0:
        # Una cuenta de IVA elegida manualmente SIEMPRE tiene prioridad, incluso si el
        # perfil tributario de la empresa quedó marcado por error como no responsable.
        # Esto evita que el sistema absorba silenciosamente el IVA en el gasto cuando
        # el documento y la decisión profesional indican IVA descontable.
        cta_manual_iva = None
        if str(cuentas_control.get("iva_descontable") or "").strip():
            cta_manual_iva = _resolver_cuenta_control(
                db, empresa, factura, cuenta_gasto, "iva_descontable", cuentas_control,
                empresa.cuenta_iva_descontable_id
            )
        if cta_manual_iva:
            lineas.append(LineaPartida(cta_manual_iva.id, cta_manual_iva.codigo, cta_manual_iva.nombre, "debito", iva, descripcion))
        elif empresa.responsable_iva:
            # Prioridad automática: regla manual aprendida > cuenta por tasa/base >
            # historial/nombre > cuenta global de respaldo.
            cta = _buscar_regla_control_manual(db, empresa, factura, cuenta_gasto, "iva_descontable")
            if not cta:
                base_iva = float(factura.base_gravable or subtotal or 0)
                cta = _seleccionar_cuenta_iva_por_tasa(db, empresa, base_iva, iva, factura.concepto_resumen,
                                                        tipo_iva="descontable")
            if not cta:
                cta = _resolver_cuenta_control(db, empresa, factura, cuenta_gasto, "iva_descontable", {}, empresa.cuenta_iva_descontable_id)
            if not cta:
            ...
            # Si el perfil realmente es NO responsable de IVA y no hay corrección
            # profesional, el IVA se trata como mayor valor del gasto/costo.
            lineas[0].valor += iva
...
def _generar_partida_venta(db: Session, empresa: Empresa, factura: Factura,
    ...
    if iva > 0:
        cta_manual_iva = None
        if str(cuentas_control.get("iva_generado") or "").strip():
            cta_manual_iva = _resolver_cuenta_control(
                db, empresa, factura, cuenta_ingreso, "iva_generado", cuentas_control,
                empresa.cuenta_iva_generado_id
            )
        if cta_manual_iva:
            lineas.append(LineaPartida(cta_manual_iva.id, cta_manual_iva.codigo, cta_manual_iva.nombre, "credito", iva, descripcion))
        elif empresa.responsable_iva:
            cta = _buscar_regla_control_manual(db, empresa, factura, cuenta_ingreso, "iva_generado")
            if not cta:
                base_iva = float(factura.base_gravable or subtotal or 0)
                cta = _seleccionar_cuenta_iva_por_tasa(db, empresa, base_iva, iva,
                                                        factura.concepto_resumen, tipo_iva="generado")
            if not cta:
                cta = _resolver_cuenta_control(db, empresa, factura, cuenta_ingreso, "iva_generado", {}, empresa.cuenta_iva_generado_id)
            if not cta:
...
def _generar_partida_nomina_multilinea(db: Session, empresa: Empresa, factura: Factura,
    ...
    if salario > 0:
        if cta_salario:
            ...
            errores.append("Falta configurar la cuenta de Salario en Historial contable → Cuentas de nómina.")
    ...
    if transporte > 0:
        ...
        if cta_transporte:
            ...
            errores.append("Falta configurar la cuenta de Auxilio de transporte en Historial contable → Cuentas de nómina.")

    if salud > 0:
        cta_salud = cta(empresa.cuenta_salud_por_pagar_id)
        if not cta_salud:
            errores.append("Falta configurar la cuenta de Salud por pagar en Historial contable → Cuentas de nómina.")
        elif not empleado.eps_nit:
            errores.append(f"Falta el NIT de la EPS en la ficha del empleado {nombre_empleado} (Historial contable → Empleados).")
        else:
    ...
    if pension > 0:
        ...
        if not cta_pension:
            errores.append("Falta configurar la cuenta de Pensión por pagar en Historial contable → Cuentas de nómina.")
        elif not empleado.afp_nit:
            errores.append(f"Falta el NIT del fondo de pensión en la ficha del empleado {nombre_empleado} (Historial contable → Empleados).")
        else:
    ...
    if neto_a_pagar > 0:
        ...
        if cta_nomina_pagar:
            ...
            errores.append("Falta configurar la cuenta de Nómina por pagar en Historial contable → Cuentas de nómina.")
...
def persistir_partida(db: Session, empresa_id: str, factura: Factura,
    ...
    db.query(Movimiento).filter(Movimiento.empresa_id == factura.empresa_id, Movimiento.factura_id == factura.id).delete()
