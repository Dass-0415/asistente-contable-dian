...
def _clave_desde_nombre(nombre: str) -> str:
    ...
    cuando no se puede determinar CUFE/CUDE (ej. PDF sin XML asociado)."""
...
def _anexar_pdf(doc: DocumentoExtraido, nombre_pdf: str, contenido_pdf: bytes, resultado_pdf: Optional[dict] = None) -> None:
    """Adjunta y contrasta el PDF con el XML sin ejecutar OCR innecesario.

    V10.8.2: el PDF ya no es un soporte pasivo cuando existe XML. Se lee su
    texto embebido (sin OCR) para confirmar CUFE/CUDE, número, NIT, fecha y
    total. Esto permite cruzar XML + PDF + Excel y detectar inconsistencias
    sin convertir una carga normal en un proceso pesado.
    """
    doc.nombre_pdf = nombre_pdf
    # El contenido binario no se conserva después de extraer sus campos. En
    # lotes grandes retener cada PDF duplicaba el uso de RAM sin aportar nada:
    # _anexar_pdf ya deja CUFE/CUDE, total, fecha, NIT y demás datos de control.
    doc.pdf_bytes = None
    ...
    mapa = {
        "cufe": "_pdf_identificador",
        "tipo_identificador_fiscal": "_pdf_tipo_identificador",
        "total": "_pdf_total",
        "naturaleza_documento": "_pdf_naturaleza",
        "numero_factura": "_pdf_numero_factura",
        "nit_emisor": "_pdf_nit_emisor",
        "fecha_emision": "_pdf_fecha_emision",
        "subtipo_documento": "_pdf_subtipo_documento",
    }
    for origen, destino in mapa.items():
        if campos_pdf.get(origen) not in (None, ""):
            doc.campos[destino] = campos_pdf.get(origen)
...
def agrupar_documentos(pares: list[tuple[str, bytes]], nit_empresa: Optional[str] = None) -> list[DocumentoExtraido]:
    ...
    for nombre_xml, contenido in xmls:
        ...
        if naturaleza_raiz == "invalido":
            errores.append(DocumentoExtraido(
                clave_agrupacion=clave_archivo, nombre_xml=nombre_xml, xml_bytes=None,
        ...
        if not resultado["ok"]:
            errores.append(DocumentoExtraido(
                ...
                xml_bytes=None, error=resultado["error"], nombre_origen=nombre_xml,
            ))
            continue

        identificador = resultado["campos"].get("cufe") or ""
        clave = identificador.lower() if identificador else clave_archivo

        nit_emisor = _nit_base(resultado["campos"].get("nit_emisor"))
        nit_receptor = _nit_base(resultado["campos"].get("nit_receptor"))
        direccion = "no_aplica"
        if nit_empresa_norm and nit_emisor and nit_emisor == nit_empresa_norm:
            direccion = "emitida"
        elif nit_empresa_norm and nit_receptor and nit_receptor == nit_empresa_norm:
            direccion = "recibida"

        naturaleza_extraida = resultado["campos"].get("naturaleza_documento") or naturaleza_raiz
        doc = DocumentoExtraido(
            clave_agrupacion=clave, nombre_xml=nombre_xml, xml_bytes=None,
            fuente_extraccion="xml", confianza=100.0, campos=resultado["campos"],
            naturaleza=naturaleza_extraida, direccion=direccion,
        )
    ...
    for clave_archivo, (nombre_pdf, contenido_pdf) in list(pdf_pendientes.items()):
        ...
        # se intenta extraer su CUFE/CUDE del propio texto: si coincide con
        # el identificador de un XML ya procesado, es el MISMO documento con un nombre
        ...
        resultado_pdf = extraer_factura_pdf(contenido_pdf, permitir_ocr=False)
        identificador_pdf = (resultado_pdf["campos"].get("cufe") or "").lower()
        if identificador_pdf and identificador_pdf in documentos:
            doc_existente = documentos[identificador_pdf]
    ...
    for clave_archivo, (nombre_pdf, contenido_pdf) in pdf_pendientes.items():
        ...
        nit_receptor_pdf = _nit_base(campos_pdf.get("nit_receptor"))
        direccion_pdf = "no_aplica"
        if nit_empresa_norm and nit_emisor_pdf and nit_emisor_pdf == nit_empresa_norm:
            direccion_pdf = "emitida"
        elif nit_empresa_norm and nit_receptor_pdf and nit_receptor_pdf == nit_empresa_norm:
            direccion_pdf = "recibida"
        clave_pdf = (campos_pdf.get("cufe") or "").strip().lower() or clave_archivo
        documentos[clave_pdf] = DocumentoExtraido(
            clave_agrupacion=clave_pdf, nombre_pdf=nombre_pdf, pdf_bytes=None,
