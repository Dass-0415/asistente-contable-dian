...
def _inicio_para_tipo(tipo_documento: str, consecutivo_inicial: int | None,
                     consecutivos_iniciales: dict[str, int] | None = None) -> int:
    """Inicio independiente por tipo documental; nunca comparte secuencia entre clases DIAN."""
    mapa = consecutivos_iniciales or {}
    return _inicio_para_config(mapa.get(tipo_documento, consecutivo_inicial))


def proyectar_numeros(db: Session, empresa: Empresa, facturas_ordenadas: list[Factura],
                      consecutivo_inicial: int | None = 1,
                      consecutivos_iniciales: dict[str, int] | None = None) -> dict[str, str]:
    """Numeración de vista previa, totalmente sin memoria y separada por tipo DIAN.

    Cada tipo documental (factura recibida, nota crédito recibida, factura emitida,
    etc.) lleva su propia secuencia. Si usa ``folio_dian`` no consume ni solicita
    consecutivo interno.
    """
    ...
    for f in facturas_ordenadas:
        tipo_doc = tipo_documento_clave(f)
        cfg = dict(cfgs[tipo_doc])
        ...
        if cfg.get("modo_numeracion") == "folio_dian":
            ...
            continue
        inicio = _inicio_para_tipo(tipo_doc, consecutivo_inicial, consecutivos_iniciales)
        ordinal = usados.get(tipo_doc, 0)
        resultado[f.id] = str(inicio + ordinal)
        usados[tipo_doc] = ordinal + 1
    return resultado


def asignar_numeros(db: Session, empresa: Empresa, facturas_ordenadas: list[Factura],
                    consecutivo_inicial: int | None = 1,
                    consecutivos_iniciales: dict[str, int] | None = None) -> tuple[dict[str, str], dict[str, dict]]:
    ...
    """
    cfgs = configuraciones_empresa(db, empresa)
    numeros = proyectar_numeros(db, empresa, facturas_ordenadas, consecutivo_inicial, consecutivos_iniciales)
