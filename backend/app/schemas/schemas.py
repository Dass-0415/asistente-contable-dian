...
class EmpresaCreate(BaseModel):
    ...
    periodicidad_iva: str = "bimestral"
    agente_retencion: bool = False
    obligado_renta: bool = False
    obligado_exogena: bool = False
    obligado_ica: bool = False
    municipio_ica: Optional[str] = None


class EmpresaPerfilTributarioUpdate(BaseModel):
    tipo_persona: str
...
class VencimientoTributarioCreate(BaseModel):
    obligacion: str
...
class FacturaOut(BaseModel):
    ...
    relacion_documento_confianza: Optional[str] = None
    relacion_documento_motivo: Optional[str] = None
...
class RelacionDocumentoRequest(BaseModel):
    documento_origen_id: Optional[str] = None


class AplicarRetencionesRequest(BaseModel):
    tipos: List[str] = Field(default_factory=list)
    cuenta_principal_codigo: Optional[str] = None
    confirmar_media: bool = False
...
class GenerarExportacionRequest(BaseModel):
    ...
    consecutivo_inicial: int = Field(default=1, ge=1)  # compatibilidad con versiones anteriores
    consecutivos_iniciales: Dict[str, int] = Field(default_factory=dict)  # por tipo documental SIIGO
...
class SugerenciaCuenta(BaseModel):
    ...
    cuenta_candidata: Optional[str] = None
    motivo: str
