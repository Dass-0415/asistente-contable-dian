...
from sqlalchemy import (
    ...
    UniqueConstraint, Index, Enum as SAEnum, Numeric, Date
)
...
class Empresa(Base):
    ...
    periodicidad_iva = Column(String(20), nullable=False, default='bimestral')
    agente_retencion = Column(Boolean, nullable=False, default=False)
    obligado_renta = Column(Boolean, nullable=False, default=False)
    obligado_exogena = Column(Boolean, nullable=False, default=False)
    obligado_ica = Column(Boolean, nullable=False, default=False)
    municipio_ica = Column(String(120), nullable=True)
...
class VencimientoTributario(Base):
    """Vencimiento manual/territorial asociado a una empresa.

    Los vencimientos nacionales DIAN conocidos se calculan en tiempo real
    desde el perfil tributario. Esta tabla conserva fechas territoriales o
    especiales que dependen de municipio/situaciones particulares.
    """
    __tablename__ = "vencimientos_tributarios"

    id = Column(String(36), primary_key=True, default=_uuid)
    empresa_id = Column(String(36), ForeignKey("empresas.id"), nullable=False, index=True)
    obligacion = Column(String(120), nullable=False)
    periodo = Column(String(80), nullable=True)
    fecha_vencimiento = Column(Date, nullable=False, index=True)
    jurisdiccion = Column(String(120), nullable=True)
    fuente = Column(String(300), nullable=True)
    activo = Column(Boolean, nullable=False, default=True)
    creado_en = Column(DateTime(timezone=True), default=_now, nullable=False)

    __table_args__ = (
        Index("ix_vencimiento_empresa_fecha", "empresa_id", "fecha_vencimiento"),
    )


class RutEmpresa(Base):
    """Análisis estructurado de un RUT cargado por el usuario.

    Se conserva el historial de análisis para detectar cambios entre versiones,
    pero no se guarda el PDF ni datos personales innecesarios (correo, teléfono,
    dirección o representantes). Solo información tributaria útil.
    """
    __tablename__ = "ruts_empresa"

    id = Column(String(36), primary_key=True, default=_uuid)
    empresa_id = Column(String(36), ForeignKey("empresas.id"), nullable=False, index=True)
    archivo_nombre = Column(String(300), nullable=True)
    archivo_sha256 = Column(String(64), nullable=False, index=True)
    numero_formulario = Column(String(40), nullable=True)
    nit_detectado = Column(String(30), nullable=True)
    dv_detectado = Column(String(2), nullable=True)
    tipo_persona_detectado = Column(String(20), nullable=True)
    direccion_seccional = Column(String(160), nullable=True)
    municipio = Column(String(160), nullable=True)
    fecha_generacion = Column(Date, nullable=True)
    actividades_json = Column(Text, nullable=False, default="[]")
    responsabilidades_json = Column(Text, nullable=False, default="[]")
    fuente_extraccion = Column(String(20), nullable=False, default="pdf_texto")
    activo = Column(Boolean, nullable=False, default=True)
    analizado_en = Column(DateTime(timezone=True), default=_now, nullable=False)
    aplicado_en = Column(DateTime(timezone=True), nullable=True)

    __table_args__ = (
        Index("ix_rut_empresa_activo", "empresa_id", "activo"),
        Index("ix_rut_empresa_analizado", "empresa_id", "analizado_en"),
    )
...
class Factura(Base):
    ...
    retenciones_json = Column(Text, nullable=True)      # Retenciones reportadas por la fuente DIAN/XML
    retenciones_contables_json = Column(Text, nullable=True)  # Retenciones sugeridas/confirmadas por el contador, sin alterar la fuente DIAN
    ...
    # Relación documental (nota crédito/débito -> factura original). Se guarda sin FK
    # física para no bloquear eliminaciones administrativas; la aplicación valida
    # siempre que el documento relacionado pertenezca a la misma empresa.
    documento_origen_id = Column(String(36), nullable=True, index=True)
    relacion_documento_confianza = Column(String(20), nullable=True)  # alta | media | manual
    relacion_documento_motivo = Column(Text, nullable=True)
